...
<head>
    <meta charset="utf-8">
    <title>Facture {{ $commande->numero_commande ?? 'CMD-'.$commande->id }}</title>
    <style>
...
<body>
    <div class="top-strip">
        {{ $boutique->nom }} &middot; Facture de commande
    </div>
            ...
            <tr>
                ...
                <td width="38%" style="vertical-align: middle; text-align: right;">
                    ...
                    <div style="margin-top: 6px;">
                        <span class="ref-badge">N° {{ $commande->numero_commande ?? 'CMD-'.$commande->id }}</span>
                    </div>
                </td>
            </tr>
        </table>
        ...
        <tr>
            <td width="50%">
                ...
                <div style="font-weight: bold; font-size: 12px;">
                    {{ optional($commande->date_commande)->format('d/m/Y') ?? $commande->created_at->format('d/m/Y') }}
                </div>
                @if($commande->date_livraison_prevue)
                    <div class="muted small" style="margin-top: 3px;">Livraison prévue: {{ $commande->date_livraison_prevue->format('d/m/Y') }}</div>
                @endif
                <div style="margin-top: 5px;">
                    <span class="ref-badge" style="background: #f59e0b;">{{ ucfirst(str_replace('_', ' ', $commande->statut)) }}</span>
                </div>
            </td>
            <td width="50%">
                <div class="info-label">Créée par</div>
                <div style="font-weight: bold; font-size: 12px;">
                    {{ $commande->user?->name ?? 'Gérant' }}
                </div>
        ...
        <tbody>
            ...
            @foreach($commande->lignes as $ligne)
                ...
                <tr class="{{ $i % 2 === 0 ? 'alt' : '' }} {{ $loop->last ? 'last' : '' }}">
                    ...
                    <td class="num">{{ number_format($ligne->montant_total, 0, ',', ' ') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <table class="totals-box">
        <tr>
            ...
            <td width="120" class="row-value" style="font-size: 13px;">{{ number_format($commande->montant_total, 0, ',', ' ') }} FCFA</td>
        </tr>
        @if($commande->montant_paye > 0)
        <tr>
            <td class="row-label">Montant payé</td>
            <td width="120" class="row-value" style="color: #16a34a;">- {{ number_format($commande->montant_paye, 0, ',', ' ') }} FCFA</td>
        </tr>
        <tr>
            <td class="row-label">Reste à payer</td>
            <td width="120" class="row-value">{{ number_format($commande->montant_restant, 0, ',', ' ') }} FCFA</td>
        </tr>
        @endif
        <tr class="grand-total">
            <td class="row-label">TOTAL {{ \Illuminate\Support\Str::upper($boutique->devise ?? 'XOF') }}</td>
            <td width="120" class="row-value">{{ number_format($commande->montant_total, 0, ',', ' ') }} {{ $boutique->devise }}</td>
        </tr>
    </table>
    ...
    <table class="payment" width="100%">
        <tr>
            <td class="left" width="100%">
                @if(!empty($commande->mode_paiement))
                    <div class="info-label">Mode de paiement</div>
                    <div style="font-weight: bold; text-transform: capitalize;">{{ str_replace('_', ' ', $commande->mode_paiement) }}</div>
                @else
