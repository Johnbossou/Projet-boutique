...
export default defineRailway(() => {
  ...
  const storageVolume = volume("sgci-storage", { region: "ams", sizeMB: 500 });
  const sgciBackend = service("sgci-backend", {
    source: github("Johnbossou/Projet-boutique", { rootDirectory: "sgci-backend" }),
    build: { buildEnvironment: "V3", builder: "DOCKERFILE", dockerfilePath: "Dockerfile" },
    replicas: { "ams": 1 },
    volumeMounts: { "/var/www/html/storage/app/public": storageVolume },
  ...
  return project("miraculous-charisma", {
    resources: [sgciBackend, MySQL, mysqlVolume, storageVolume],
  });
});
